...
const migrateProducts = async (version) => {
  try {
    const products = await Product.getProducts({ version: version });
    if (products.length > 0) return { noNeed: true };
    await Product.delete({});

    console.log(__dirname);
    const confPath = path.join(__dirname, '..', '..', 'static', 'products.json');
    const productsConfig = JSON.parse(await fs.readFileSync(confPath));
    console.log(productsConfig);
    await Promise.all(
      productsConfig.map(async (plan) => {
        await Product.saveProduct({ ...plan, version: version });
      })
    );
  } catch (error) {
    console.log(error);
    return { message: '[Migrate] Error migrating products' };
  }
};
...
async function migrateDb() {
  ...
  ret[2] = await migrateProducts(process.env.PRODUCT_VERSION);
